...
\begin{frame}{From states to trees}
  \justifying
  We now turn to a different formalism, \textbg{born in videogame AI} and now \textbg{ubiquitous in robotics}: the \textbg{behavior tree} (BT).\\
  \bigskip
  Behavior trees are
  \begin{itemize}
...
% --- Leaf nodes ---
\begin{frame}{Leaf nodes}{Overview}
  \justifying
  Leaves are where the tree interacts with the rest of the system.
  \begin{itemize}
...
\begin{frame}{Control nodes}{Overview}
  \justifying
  \texttt{BT.CPP} provides a variety of \textbg{nodes that control the execution flow}.\\
  Just some examples that are most frequently used...
  \begin{itemize}
    ...
    \item \textbg{IfThenElse}: evaluate a condition on the first branch, then execute the second or the third.
    \item \textbg{TryCatch}: execute the first N-1 branches, fall back to the N-th in case of an error.
  \end{itemize}
  \bigskip
  Sequence, Fallback, IfThenElse have \textbg{plain} and \textbg{reactive} variants, with subtly different semantics.
\end{frame}
\begin{frame}{Control nodes}{Sequence and Fallback}
  ...
  \begin{block}{Memory of plain controls}
    \justifying
    On a re-tick after \texttt{RUNNING}, \textbf{plain controls resume from the running child}, skipping the children that have already completed (\emph{i.e.}, they \textbf{do not re-check previous conditions}).
  \end{block}
\end{frame}
...
\begin{frame}{Control nodes}{Reactive variants}
  \justifying
  \textbg{ReactiveSequence}, \textbg{ReactiveFallback}, and \textbg{WhileDoElse} differ from their plain siblings on one point: on a re-tick, they do not skip the already-completed children.\\
...
\begin{frame}{Control nodes}{Parallel and ParallelAll}
  ...
  \bigskip
  \textbg{They are the only nodes that can have multiple \texttt{RUNNING} children at the same time.}\\
  \bigskip
  ...
  \bigskip
  Both are the right tool when \textbg{several independent behaviors must progress together} --- for example, navigate while keeping an eye on the battery and on the environment.
\end{frame}
\begin{frame}{Control nodes}{Parallel and ParallelAll}
  \begin{figure}
    \centering
    \begin{tikzpicture}[
      ...
      every node/.style={font=\footnotesize},
      ctrl/.style={draw, rounded corners=3pt, fill=blue!15, minimum width=2.1cm, minimum height=0.6cm, thick, align=center},
      act/.style={draw, fill=green!15, minimum width=1.75cm, minimum height=0.55cm, thick},
      cond/.style={draw, rounded corners=8pt, fill=orange!20, minimum width=1.75cm, minimum height=0.55cm, thick},
      edge from parent/.style={draw, thick}
    ]
      \node[ctrl] {Parallel\\$s=3$, $f=1$}
      child {node[cond] {SystemOK?}}
      child {node[cond] {BatteryOK?}}
      child {node[act]  {TrackTarget}}
      child {node[act]  {AvoidObstacles}}
      child {node[act]  {UpdateMap}};
    \end{tikzpicture}
    \caption{A \texttt{Parallel} node with thresholds \texttt{min\_success=3} and \texttt{min\_failure=1}: the subtree succeeds as soon as any three children succeed (supposedly, the first three), and fails as soon as any child fails; every remaining \texttt{RUNNING} child is halted.}
  \end{figure}
\end{frame}
\begin{frame}{Control nodes}{Concurrency, not parallelism}
  ...
  \begin{alertblock}{The right mental model}
    ...
    \textbr{Long would-be-blocking work belongs in an asynchronous action}, never inside the tick of a synchronous one (unless specific exceptions, see later).
  \end{alertblock}
\end{frame}
...
\begin{frame}{Decorators}
  ...
  \begin{itemize}
    ...
    \item \textbg{KeepRunningUntilFailure}: return \texttt{RUNNING} instead of \texttt{SUCCESS}.
  \end{itemize}
  \bigskip
...
\begin{frame}{The executor pattern}{\texttt{transitions\_ros} vs \texttt{dua\_behaviortree\_cpp}}
  ...
  \begin{block}{}
    \centering
    Same pattern, two formalisms: generic executor, logic loaded at runtime.
  \end{block}
\end{frame}

% --- Use case: Leonardo Drone Contest ---
\begin{frame}{Use case: Leonardo Drone Contest}{Overview of LDC25}
  \begin{columns}
    \column{.5\textwidth}
    \justifying
    \begin{itemize}
      \item \textbg{Localize} ArUco, QR, COCO objects.
      \item Execute \textbg{tasks} within \textbg{deadlines}.
        \begin{itemize}
          \item \textbg{FindTarget} (with pic).
          \item \textbg{FollowSequence} (with pic).
          \item \textbg{FindObject} (UGV, with pic).
          \item \textbg{Loiter} (UAV, with video).
          \item \textbg{PTZCollab} (with pic).
          \item \textbg{LandOnHighestSpot} (UAV, with pic).
          \item \textbr{EmergencyLanding} (UAV).
          \item \textbr{ReturnToBase} (UGV).
        \end{itemize}
    \end{itemize}
    ...
    \begin{figure}
      \centering
      \includegraphics[width=\linewidth]{ldc25_intro.jpg}
      \caption{Corinna (UAV) and DottorCane (UGV).}
      \label{fig:ldc25intro}
    \end{figure}
  \end{columns}
\end{frame}
\begin{frame}{Use case: Leonardo Drone Contest}{BT design choices of LDC25}
  \justifying
  \begin{itemize}
    \item Deploy a \textbg{BT on each agent}, both sharing a \textbg{similar structure}.
    \item Code \textbg{tasks} into a \textbg{reusable custom nodes library}.
    \item Wrap a \textbg{task scheduling subtree} inside an agent-specific BT which handles safe initialization and shutdown.
  \end{itemize}
\end{frame}
\begin{frame}{Use case: Leonardo Drone Contest}{BT diagram of LDC25}
  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{ldc25_btstruct.pdf}
    \caption{High-level view of the agent-agnostic BT structure: the Lifecycle Root level waits for mission start, arms, then finally disarms the robot; the Mission Coordination level initializes the agent (\emph{e.g.}, takes off), performs the mission watching for a stop command, and finalizes the agent (\emph{e.g.}, lands safely); the Task Dispatch level runs until halted, selecting and executing tasks issued by the Ground Control Station.}
    \label{fig:ldc25btstruct}
  \end{figure}
\end{frame}
\begin{frame}{Use case: Leonardo Drone Contest}{Strategy of LDC25}
  \justifying
  \begin{itemize}
    \item At each tick, \textbg{Task Scheduler selects the best task node} which is then ticked.
    \item If the scheduling selection changes, \textbg{preemption} is achieved by \textbg{halting} the running node.
    \item The \textbg{scheduling policy} is based on three \textbg{tiers}.
      \begin{enumerate}
        \item \textbg{Emergency tasks:} non-preemptable \texttt{SyncActionNodes}, always selected first.
        \item \textbg{Tasks with non-preemptable portions:} \texttt{StatefulActionNodes} that may retain control (\emph{i.e.}, \texttt{onRunning()} does not return until a critical operation is completed).
        \item \textbg{All other tasks}: evaluated by a cost function with a hysteresis margin.
      \end{enumerate}
    \item Tier 3 cost function evaluates \textbg{distance} and an \textbg{educated guess of the score}.
    \item If no valid task is available, an \textbg{idle task} of \textbg{persistent exploration} is executed.
    \item \textbg{Agents share information} on detected targets: each Task Scheduler keeps a \textbg{database of found targets}, used to \textbg{navigate directly to known targets} when relevant tasks are issued.
  \end{itemize}
\end{frame}
\begin{frame}{Use case: Leonardo Drone Contest}{Task scheduling and execution performance of LDC25}
  \begin{figure}
    \centering
    \includegraphics[width=.86\linewidth]{ldc25_tasksresult.pdf}
    \label{fig:ldc25taskresult}
  \end{figure}
\end{frame}
\begin{frame}{Use case: Leonardo Drone Contest}{Task scheduling and execution performance of LDC25}
  \justifying
  \begin{itemize}
    \item Over a 20 minutes manche, 23 tasks were issued and 20 were completed successfully, yielding a \textbg{completion rate of 87\%}.
    \item Thanks to the \textbg{idle exploration task}, 13 out of 19 valid manche targets were \textbg{found in the first \SI{60}{\second}}, requiring \textbg{10 times less time}.
    \item \textbg{Emergency tasks} were always \textbg{scheduled immediately} and completed successfully.
  \end{itemize}
\end{frame}
